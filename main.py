...
def train_cross_validation(model, fold_i, train_loader, test_loader):
    ...
    train_rsme_numpy, test_rsme_numpy, test_pcc_numpy = np.zeros((max_epoches)), np.zeros((max_epoches)), np.zeros((max_epoches))
    ...
    while epoch < max_epoches:
        ...
        train_rsme_numpy[epoch] = train_sqrt_loss

        if (epoch + 1) % val_iter == 0:
            test_rsme, test_pcc_value, affinity_label, affinity_predict = test_pcc(model, test_loader)
            test_rsme_numpy[epoch] = test_rsme
            test_pcc_numpy[epoch] = test_pcc_value
            print('\n[TEST] seed = %d, pcc: %f, rmse: %f' % (seed, test_pcc_value, test_rsme))
            if test_pcc_value >= best_pcc and test_rsme <= best_rmse:
            ...
            if not_best_count >= early_stop:
                break
        epoch += 1
    dump((train_rsme_numpy, test_rsme_numpy, test_pcc_numpy), result_path + 'cross_validation_fold' + str(fold_i))
    return train_rsme_numpy, test_rsme_numpy, test_pcc_numpy, best_pcc, best_rmse
...
if __name__ == '__main__':
    ...
    train_rsme_10fold, test_rsme_10fold, test_pcc_10fold = [], [], []
    ...
    for fold_i in range(10):   # begin ten-fold cross validation training
        ...
        train_rsme_numpy, test_rsme_numpy, test_pcc_numpy, best_pcc, best_rmse = train_cross_validation(model, fold_i, train_loader, test_loader)
    ...
    dump((train_rsme_10fold, test_rsme_10fold, test_pcc_10fold), result_path + '10fold_cross_validation_result')
